...
def updateLR(fileLocation):
    ...
    validA = changeData(validA, trainingDiffWords)

    #weights = np.zeros(trainingA.shape[1])
    ...
    totalTrainingDataX = changeData(totalTrainingDataX, totalTrainingDiffWords)
...
def changeData(dataset, diffWords):
    setOfDataProcessed = pd.DataFrame(columns=diffWords)
    lengthOfDiffWords = len(diffWords)
    for idx, row in dataset.iterrows():
        data = [0] * lengthOfDiffWords
...
if __name__ == '__main__':
    ...
    print("Scores for Logistic Regression-Bag Of Words")
